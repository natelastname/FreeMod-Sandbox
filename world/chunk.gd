...
func _ready():
	...
	# data is a hash table that maps a position vector to an int from 1 to 29.
	# The position is the position of a block and the int is the block's type.
	if Settings.world_type == 0:
		data = TerrainGenerator.random_blocks(chunk_position)
	else:
